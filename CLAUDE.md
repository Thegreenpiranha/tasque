...
- **Layer boundaries:** Widgets and screens do not import `db.py` directly — they go through the **controller**. Data flows `db → controller → widget`; user actions flow `widget → controller → db`. Widgets communicate upward via Textual **messages** (`Message` subclasses), not by reaching into parents. Domain **error types** (`TasqueError` and its subclasses) are the one sanctioned exception to "no `db.py` import" in the UI layer: they are **re-exported from `controller.py`**, so screens catch them via `from tasque.controller import TasqueError`. Importing them straight from `db.py` in a widget/screen is still disallowed — go through the controller re-export so the UI's dependency arrow points at the controller, never at the persistence layer.
